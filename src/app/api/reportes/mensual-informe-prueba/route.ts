...
// =====================================================
// GET /api/reportes/mensual-informe-prueba
// =====================================================
// Endpoint TEMPORAL de prueba (sin autenticación) para enviar el informe
// mensual de EJEMPLO a jsa@jsadr.com.co. Permite validar el formato del
// informe antes de que el cron automático comience a enviar el día 1 de
// cada mes.
//
// ⚠️ EN PRODUCCIÓN: este endpoint debe eliminarse o protegerse con ADMIN.
//
// Query params:
//   ?enviar=false        → NO envía el correo, solo retorna el HTML (default: true = envía)
...
export async function GET(req: NextRequest) {
  try {
    ...
    // (En producción debe protegerse con requireRole ADMIN)

    const { searchParams } = new URL(req.url)
    const enviar = searchParams.get('enviar') !== 'false' // por defecto envía
    ...
    // --- Solicitudes cancelados (liquidados) en el mes ---
    ...
    if (enviar) {
      ...
      const resultado = await enviarEmail({
        to: para,
        subject: `[JSADR] Informe Mensual — ${nombreMes} (EJEMPLO)`,
        html,
